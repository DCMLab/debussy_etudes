...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/debussy_etudes.git
...
* `Claude Debussy – Douze Études <https://github.com/DCMLab/debussy_etudes/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the first etude, *Pour les "cinq doigts", d'après monsieur Czerny*, has the following files:

-  ``MS3/l136-01_etudes_cinq.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/l136-01_etudes_cinq.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l136-01_etudes_cinq.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/l136-01_etudes_cinq.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/l136-01_etudes_cinq.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/l136-01_etudes_cinq.tsv')
   notes = ms3.load_tsv('notes/l136-01_etudes_cinq.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/debussy_etudes/issues>`__ and feel free to fork and submit pull requests.
